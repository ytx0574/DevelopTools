...
+ (NSString *)convertUnique:(NSString *)unique keys:(NSDictionary *)keys;
{
    if (!keys) {
        keys = CONVERT_UNIQUE_KEYS;
    }
    if ([[keys allKeys] containsObject:unique]) {
        unique = keys[unique];
    }
    return unique;
}
...
+ (instancetype)initWithProperties:(NSDictionary *)info
{
    id instance = [[[self class] alloc] init];
    [instance setUpWithProperties:info];
    return instance;
}

- (void)setUpWithProperties:(NSDictionary *)info
{
...
{
    //两个时间的时间差
    NSInteger TimeDifference=abs([date timeIntervalSinceDate:[NSDate date]]);
...
- (NSString *)getDateFromSecond:(NSInteger)second
{
    if (second > 600 && second % 60 >= 10) {
        ...
        return [NSString stringWithFormat:@"00:%ld", second % 60];
    }else{
