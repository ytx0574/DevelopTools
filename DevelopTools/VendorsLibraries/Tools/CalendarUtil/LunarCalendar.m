...
- (void)InitializeValue
{
	...
	NSInteger dayCyclical = (NSInteger)(([components day] + 30) / (86400 / (3600*24))) + 10;

	NSInteger sumdays = (NSInteger)[components day];
...
- (NSString *)getChineseHoliday:(NSInteger)aMonth day:(NSInteger)aDay
{
    ...
    return [chineseHolidayDict objectForKey:[NSString stringWithFormat:@"%@|%@", @(aMonth), @(aDay)]];
}

- (NSString *)getWorldHoliday:(NSInteger)aMonth day:(NSInteger)aDay
{
    NSString *monthDay;
    if(aMonth < 10 && aDay < 10)
    {
        monthDay = [NSString stringWithFormat:@"0%li0%li",(long)aMonth,(long)aDay] ;
    }
    else if(aMonth < 10 && aDay > 9)
    {
        monthDay = [NSString stringWithFormat:@"0%li%li",(long)aMonth,(long)aDay] ;
    }
    else if(aMonth > 9 && aDay < 10)
    {
        monthDay = [NSString stringWithFormat:@"%li0%li",(long)aMonth,(long)aDay] ;
    }
    else
    {
        monthDay = [NSString stringWithFormat:@"%li%li",(long)aMonth,(long)aDay] ;
    }
...
- (NSString *)getWeekHoliday:(NSInteger)aYear month:(NSInteger)aMonth day:(NSInteger)aDay
{
    ...
    for (id key in [dict allKeys]) {
        ...
        NSInteger dictWeek=[[[key substringFromIndex:2] substringToIndex:1] intValue];
        NSInteger dictDayInWeek=[[[key substringFromIndex:3] substringToIndex:1] intValue];
...
- (NSString *)getWeekDay:(NSInteger)aYear month:(NSInteger)aMonth week:(NSInteger)aWeek dayInWeek:(NSInteger)aDay
{
    NSString* dateStr =[NSString stringWithFormat:@"%@ 1 %@ +0000", @(aMonth), @(aYear)];
...
{
	for (NSInteger n = month * 2 - 1; n <= month * 2; n++)
	{
		CGFloat Termdays = [self Term:year:n:YES];
		CGFloat mdays = [self AntiDayDifference:year:floor(Termdays)];
		//double sm1 = floor(mdays / 100);
		NSInteger hour = (NSInteger)floor((CGFloat)[self Tail:Termdays] * 24);
		NSInteger minute = (NSInteger)floor((CGFloat)([self Tail:Termdays] * 24 - hour) * 60);
		NSInteger tMonth = (NSInteger)ceil((CGFloat)n / 2);
...
{
	//儒略日
	CGFloat juD = y * (365.2423112 - 6.4e-14 * (y - 100) * (y - 100) - 3.047e-8 * (y - 100)) + 15.218427 * n + 1721050.71301;
	
	//角度
	CGFloat tht = 3e-4 * y - 0.372781384 - 0.2617913325 * n;
	
	//年差实均数
	CGFloat yrD = (1.945 * sin(tht) - 0.01206 * sin(2 * tht)) * (1.048994 - 2.583e-5 * y);
	
	//朔差实均数
	CGFloat shuoD = -18e-4 * sin(2.313908653 * y - 0.439822951 - 3.0443 * n);
	
	CGFloat vs = (pd) ? (juD + yrD + shuoD - [self EquivalentStandardDay:y:1:0] - 1721425) : (juD - [self EquivalentStandardDay:y:1:0] - 1721425);
	return vs;
}
...
{
	//Julian的等效标准天数
	CGFloat v = (y - 1) * 365 + floor((CGFloat)((y - 1) / 4)) + [self DayDifference:y:m:d] - 2;
	
	if (y > 1582)
	{//Gregorian的等效标准天数
		v += -floor((CGFloat)((y - 1) / 100)) + floor((CGFloat)((y - 1) / 400)) + 2;
	} 
	return v;
}
...
- (NSInteger)GregorianYear
{
	...
	NSInteger ret = [[formatter stringFromDate:thisdate] intValue];
	return ret;
}

- (NSInteger)GregorianMonth
{
	NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
...
- (NSInteger)Weekday
{
	...
	return (NSInteger)[weekday weekday];
}
...
- (NSString *)Constellation
{
	...
	NSInteger intConstellation = [[formatter stringFromDate:thisdate] intValue];
